...
export const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <section className="relative overflow-hidden pt-8 pb-12 px-4 sm:px-6 lg:px-8 bg-slate-50 border-b border-slate-200">
      <div className="max-w-4xl mx-auto text-center relative z-10 space-y-5">
        ...
        {/* Main Search Input */}
        <div className="relative max-w-2xl mx-auto pt-2" ref={dropdownRef}>
          <div
            className={`relative flex items-center bg-white border rounded-2xl transition-all duration-300 shadow-sm ${isFocused
          ...
          {isFocused && query.trim().length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden text-left divide-y divide-slate-100">
              <div className="p-3 bg-slate-50 text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between">
                <span>Matching Articles ({matchedArticles.length})</span>
              ...
              {matchedArticles.length > 0 ? (
                <div>
                  {matchedArticles.map((art) => (
                    <button
                      ...
                      className="w-full p-3.5 hover:bg-orange-50/60 flex items-start gap-3 transition-colors text-left group"
                    >
                ...
                <div className="p-6 text-center text-slate-500 text-xs">
                  No matching guides found for "{query}". Try checking our categories below.
                </div>
              )}
            </div>
          )}
